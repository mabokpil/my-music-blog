...
app.get("/api/playlists", async (req, res) => {
  try {
    ...
    const order = ORDER_MAP[sort] || "rating";

    const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(query)}&type=playlist&maxResults=12&relevanceLanguage=ko&order=${order}&key=${API_KEY}`;
    ...
    const tracks = data.items
      ...
      .map((item) => ({
        track: {
          ...
          artists: [
            { name: item.snippet.videoOwnerChannelTitle || "알 수 없음" },
          ],
          album: {
